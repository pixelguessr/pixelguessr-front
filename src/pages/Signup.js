...
export default function Cadastro() {
  ...
  function checkSignUp(e) {
    ...
    if(password!==confirmPassword){
      return alert('Senhas discrepantes')
    }
    setLoading(true)
    let obj = {
      name,
          ...
          <div className="px-6 py-4">
            ...
            <form onSubmit={checkSignUp}>
              <div className="w-full mt-2">
                <input required disabled={loading} value={name} onChange={(e) => setName(e.target.value)} autoComplete="off" name="username" maxLength={10} className="block w-full px-4 py-2 mt-2 text-gray-200 placeholder-gray-400 border rounded-lg bg-gray-800 border-gray-600 dark:placeholder-gray-400 focus:ring-2 focus:ring-inset focus:ring-lime-600" type="text" placeholder="Nome de usuário" aria-label="Nome de usuário" />
              </div>
              ...
              <div className="flex items-center justify-between mt-4">
                <button disabled={loading} className="mx-auto flex justify-center px-6 py-2 w-40 text-sm font-medium tracking-wide text-white transition-colors duration-300 transform bg-lime-600 rounded-lg hover:bg-lime-500 focus:outline-none focus:ring focus:ring-opacity-50">
                  {loading ? <RotatingLines strokeColor="white" width="20"/> : 'Registrar-se'}
                </button>
              </div>
            </form>
          </div>
